...
\item Units are in roman type, except for constants such as $c$ or $h$
  ...
  the value with a protected ßspace (``\verb!~!'') or a thin space (``\verb!\,!''), and they should not be broken over two
  lines.  It is recommended to keep the factors of $c$ for masses and
  momenta, \eg $m = 3.1\gevcc$ (or $\gev c^{-2}$). However, if they are dropped this
...
\item The \% sign should not be separated from the number that precedes it: 5\%, not 5 \%. A thin space is also acceptable: 5\,\%, but should be applied consistently throughout the paper.
...
\item Hyphenation should be used where necessary to avoid ambiguity,
  ...
  (\CP-violating observables, \bquark-hadron decays, final-state radiation),
  ...
  Semileptonic, pseudorapidity, multivariate, multidimensional, and reweighted are single words and should not
  be hyphenated.
